...
#include "buggy_mobile_robot/botanbot_ackermann_drive.hpp"
...
namespace gazebo_plugins
{
  ...
  public:
    ...
    // Effort publisher
    rclcpp::Publisher<std_msgs::msg::Float32>::SharedPtr effort_;
    ...
    /// Keep lastest effort message
    std_msgs::msg::Float32 effort_value_;
    
    /// Keep lastest steering message
    std_msgs::msg::Float32 steering_value_;
    ...
    /// True to publish effort value.
    bool publish_effort_;
    
    /// True to publish steering value.
    bool publish_steering_;
  ...
  void botanbotAckermannDrive::Load(gazebo::physics::ModelPtr _model, sdf::ElementPtr _sdf)
  {
    ...
    // Advertise effort
    impl_->publish_effort_ = _sdf->Get<bool>("publish_effort", false).first;
    if (impl_->publish_effort_) {
      impl_->effort_ = impl_->ros_node_->create_publisher<std_msgs::msg::Float32>(
        "effort", qos.get_publisher_qos("effort", rclcpp::QoS(1)));

      RCLCPP_INFO(
        impl_->ros_node_->get_logger(), "Advertise effort on [%s]",
        impl_->effort_->get_topic_name());
    }
    
    // Advertise steering
    impl_->publish_steering_ = _sdf->Get<bool>("publish_steering", false).first;
    if (impl_->publish_steering_) {
      impl_->steering_pub_ = impl_->ros_node_->create_publisher<std_msgs::msg::Float32>(
        "steering", qos.get_publisher_qos("steering", rclcpp::QoS(1)));

      RCLCPP_INFO(
        impl_->ros_node_->get_logger(), "Advertise steering on [%s]",
        impl_->steering_pub_->get_topic_name());
    }
  ...
  void botanbotAckermannDrivePrivate::OnUpdate(const gazebo::common::UpdateInfo & _info)
  {
    ...
    if (publish_effort_) {
#ifdef IGN_PROFILER_ENABLE
      IGN_PROFILE_BEGIN("publish effort");
#endif
      effort_->publish(effort_value_);
#ifdef IGN_PROFILER_ENABLE
      IGN_PROFILE_END();
#endif
    }
     if (publish_steering_) {
#ifdef IGN_PROFILER_ENABLE
      IGN_PROFILE_BEGIN("publish steering");
#endif
      steering_pub_->publish(steering_value_);
#ifdef IGN_PROFILER_ENABLE
      IGN_PROFILE_END();
#endif
    }
    ...
    effort_value_.data= linear_cmd / 7;
    ...
    steering_value_.data = left_steering_angle;
    ...
    steering_value_.data = left_steering_cmd;
    ...
    steering_value_.data = steer_wheel_angle;

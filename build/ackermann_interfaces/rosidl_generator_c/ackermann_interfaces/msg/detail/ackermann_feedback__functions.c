  ...
  ackermann_interfaces__msg__AckermannFeedback__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    ackermann_interfaces__msg__AckermannFeedback * data =
      (ackermann_interfaces__msg__AckermannFeedback *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ackermann_interfaces__msg__AckermannFeedback__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          ackermann_interfaces__msg__AckermannFeedback__fini(&data[i]);
        }
        free(data);
        return false;
      }
